...
int main() {
    ...
    vc2s dp(2, vcs(100001));
    dp.at(1).at(A.at(1)) = "A";
    dp.at(1).at(B.at(1)) = "B";
    REP(i, N-1) {
        int odd = i%2;
        int even = 1-odd;
        REP(j, S) dp.at(even).at(j) = "";
        REP(j, S) {
            if (dp.at(odd).at(j) == "") continue;
            if (j+A.at(i+1) <= S) dp.at(even).at(j+A.at(i+1)) = dp.at(odd).at(j) + "A";
            if (j+B.at(i+1) <= S) dp.at(even).at(j+B.at(i+1)) = dp.at(odd).at(j) + "B";
        }
    }

    if (dp.at(N%2).at(S) != "") cout << dp.at(N%2).at(S) << endl;
