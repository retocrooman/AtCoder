#include <bits/stdc++.h>
using namespace std;
#define rep(i, n) for (ll i=0; i<(ll)(n); i++)
#define rev_rep(i, n) for (ll i=(n); i>=0; i--)
#define REP(i, n) for (ll i=1; i<=(ll)(n); i++)
#define REV_REP(i, n) for (ll i=(n); i>0; i--)
#define FOR(i, n, m) for (ll i=(n); i<=(ll)(m); i++)
#define rev_FOR(i, n, m) for (ll i=(n); i>=(ll)(m); i--)
#define ll long long
#define ull unsigned long long
#define vc vector<ll>
#define vc2 vector<vector<ll>>
#define vc3 vector<vector<vector<ll>>>
#define vcd vector<double>
#define vc2d vector<vector<double>>
#define vc3d vector<vector<vector<double>>>
#define vcs vector<string>
#define vc2s vector<vector<string>>
#define vc3s vector<vector<vector<string>>>
#define ALL(vec) vec.begin(),vec.end()
const ll INF=1LL<<60;
const ll mod = 1000000007;
const double pi=3.1415926535;
const int dx[4] = {1, 0, -1, 0};
const int dy[4] = {0, 1, 0, -1};
template<class T>bool ch_max(T &a, const T &b) {if (a<b) {a = b; return 1; } return 0;}
template<class T>bool ch_min(T &a, const T &b) {if (b<a) {a = b; return 1; } return 0;}

int main() {
    ll N,Q;
    cin >> N >> Q;
    vc A(N);
    rep(i, N) cin >> A.at(i);
    vc2 xy(N);
    rep(i, Q) {
        ll a,b;
        cin >> a >> b;
        a--;
        b--;
        xy.at(a).push_back(b);
        xy.at(b).push_back(a);
    }
    vc dp(9000);
    dp.at(0) = 1;
    vc2 root(9000);
    rep(i, N) {
        ll pos = A.at(i);
        rev_rep(j, 8888) {
            if(dp.at(j) == 1) {
                bool ok = true;
                for(ll a : root.at(j)) {
                    for(ll b : xy.at(i)) {
                        if (a == b) ok = false;
                    }
                }
                if(!ok) continue;
                if(dp.at(j+pos) == 1) {
                    cout << root.at(j+pos).size() << endl;
                    for(ll a : root.at(j+pos)) cout << ++a <<" ";
                    cout << endl;
                    cout << root.at(j).size()+1 << endl;
                    for(ll a : root.at(j)) cout << ++a << " ";
                    cout << ++i << endl;
                    return 0;
                }
                dp.at(j+pos) = 1;
                for(ll a : root.at(j)) {
                    root.at(j+pos).push_back(a);
                }
                root.at(j+pos).push_back(i);
            }
        }
    }
}