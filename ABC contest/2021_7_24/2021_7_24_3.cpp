...
int main() {
    ...
    rep(i, size) {
        char c = S.at(i);
        if(c == 'c') vec.at(0)++;
        if(c == 'h') vec.at(1) += vec.at(0);
        if(c == 'o') vec.at(2) += vec.at(1);
        if(c == 'k') vec.at(3) += vec.at(2);
        if(c == 'u') vec.at(4) += vec.at(3);
        if(c == 'd') vec.at(5) += vec.at(4);
        if(c == 'a') vec.at(6) += vec.at(5);
        if(c == 'i') vec.at(7) += vec.at(6);
        rep(j, 8) vec.at(j) %= mod; 
    }
    cout << vec.at(7) << endl;
}